<template>
  <div v-loading="loading" class="addSmoke">
...
import { addDevice } from "@/api/smoke"
  ...
  methods: {
      ...
      if (communityId) {
        dropdownNodeTree({communityId: communityId}).then(function (response) {
          if (response && response.data) {
            _this.nodeOptions = response.data.dataList
          }
        }).catch(function (a) {
          console.error(a)
        })
    ...
    handleSubmit: function (formName) {
      ...
      _this.$refs[formName].validate((valid) => {
        if (valid) {
          _this.loading = true;
          addDevice(_this.form).then(function (response) {
