  ...
  <section class="addWorker">
    ...
    <div v-loading="loading" class="content">
        ...
        <div v-if="activeIndex==0" key="a">
            ...
            <el-row>
              ...
              <el-col :span="8">
                <el-form-item label="有效时长" prop="leaseType">
                  <el-select v-model="form.leaseType" filterable clearable :disabled="true">
              ...
              <el-col :span="8">
                ...
                value-format="yyyyMMdd235959"
                :picker-options="beforePickerOptions2(form.leaseEndTime)"
                >
                </el-date-picker>
                </el-form-item>
                <el-button type="text" @click="handleRenewal">续期</el-button>
              </el-col>
            </el-row>
          </div>
              ...
              <el-row>
                <el-col :span="8">
                  <el-form-item label="出生日期" prop="birthDate">
                    <el-date-picker v-model.trim="form.birthDate" clearable  placeholder="YYYY-MM-DD">
                    </el-date-picker>
                  </el-form-item>
                </el-col>
                <el-col :span="16" style="line-height:36px">
                  <el-form-item label="证件有效期限" prop="validStartTime">
                    <el-date-picker style="width:160px"
                      v-model.trim="form.validStartTime"
                  ...
                  <el-form-item prop="validEndTime" v-if="!form.checkedValidEnd">
                    <el-date-picker style="width:160px"
                      v-model.trim="form.validEndTime"
            ...
            <el-row :gutter="20">
                  ...
                  <el-row>
                    <el-col :span="24">
                      <div class="module-title">
                        ...
                        <label class="module-title-class" @click="editCardNo()" v-if="form.cardNo">编辑门卡</label>
                        <label class="module-title-class" @click="editCardNo()" v-else>添加门卡</label>
                      </div>
                    </el-col>
                  </el-row>
                  <el-row>
                    <el-col :span="12">
                      <el-form-item label="卡类型">
                        <el-select v-model="form.cardClass" filterable placeholder="请选择卡类型" :disabled="true">
                    ...
                    <el-col :span="12">
                      <el-form-item label="门卡卡号" prop="cardNo" :rules="rules.cardNo" v-if="form.cardClass!=='2'">
                        <el-input style="width:206px;" v-model="form.cardNo" clearable placeholder="请输入卡号" maxlength="8" :type="pswVisibility?'type':'password'" :disabled="true">
                      ...
                      <el-form-item label="门卡卡号" prop="cardNo" v-else-if="form.cardClass==='2'" :rules="rules.cardNo2">
                        <el-input style="width:206px;" v-model="form.cardNo" clearable placeholder="请输入卡号" maxlength="16" :type="pswVisibility?'type':'password'" :disabled="true">
                          <i slot="prefix" class="iconfont" :class="pswVisibility?'icon-htmal5icon09':'icon-htmal5icon08'" @click="pswVisibility=!pswVisibility"></i>
                        </el-input>
                      </el-form-item>
                    </el-col>
              ...
              <el-col :span="8">
                <div class="content-doorcard_class">
                  ...
                  <el-row>
                    <el-col :span="24">
                      <component :is="faceViewName" v-on:childimgBase64="childimgBase64" ref="face" :faceUrl='form.faceUrl'/>
                    </el-col>
                  </el-row>
                </div>
          ...
          <div class="form-dialog-control-area">
              ...
              <el-button @click="handleSubmitNew()" type="primary">保存</el-button>
          </div>
        </div> 
        </el-form>
    </div>
    <div v-if="editshowCardNo">
      <el-dialog
        title="编辑门卡"
        :visible.sync="editshowCardNo"
        width="400px"
        :close-on-click-modal="false">
        <edit-cardNo :data="form" @delstatus="delstatusdata" @editform='editformdata' />
      </el-dialog>
    </div>
    <div v-if="showRenewal">
      <el-dialog
        ...
        width="400px"
        :close-on-click-modal="false">
        <renewal :data="form" @submitSuccess="handleClose()"/>
      </el-dialog>
    </div>
  </section>
</template>
<script>
import editCardNo from './components/editCardNo'//编辑
import {formatTimeMillisToDate} from "@/filters/index";
import { modifyWorker,detailWorker } from "@/api/personnel";
...
import faceWorkerLayoutfix from './components/faceWorkerLayoutfix'//人脸信息编辑
...
export default {
  ...
  methods: {
    ...
    afterPickerOptions2,
    handleRenewal: function(){//续期
      this.showRenewal = true
    },
    handleClose: function () {
      this.showRenewal = false;//续期
    },
    editCardNo:function(){//编辑门禁卡
      this.editshowCardNo = true;
    },
    delstatusdata(data){//接受编辑门禁卡传来的值
      if(data == '1'){
        this.form.cardNo = ''
        this.form.cardClass = ''
      }
    ...
    editformdata(data){
      ...
      this.form.cardClass = data.cardClass
      this.form.cardInfo = data.cardInfo
    },
    ...
    handleSubmitNew(){//保存
      ...
      self.$refs.form.validate((valid) => {
        if (valid) {
          self.loading = true;
          this.form.faceUrl = this.$refs.face.imgBase64;
          ...
          modifyWorker(data).then((response) => {
    ...
    loadData() {
      ...
      detailWorker(query).then(function (response) {
