  ...
  <div class="jdDIV">
          ...
          <el-col>
            <div class="table-control-panel">
              <div style="margin-bottom:20px">
                ...
                <span class="control-left" style="margin-left:-10px">
                  <el-button
                    type="primary"
                    class="jdaddbtn"
                    icon="el-icon-plus"
      ...
      <div class="content">
        ...
        <div v-else class="device-list" ref="deviceList">
          <div class="device-item" v-for="(device,i) in tableData" :key="i">
            <div class="item-loading" v-if="device.itemLoading"></div>
            <div class="device-body">
              ...
              <p>
                ...
                <!-- <el-button type="text" class="fr no-padding" size="small" :disabled="device.upgradeStatus != '1'" @click="handleUp(device.doorId)">
                  {{ device.upgradeStatus != '2'? '一键升级' : '升级中'}}
                </el-button>-->
              </p>
              <p class="title_p">
                ...
                <el-tooltip effect="dark" :content="device.doorName" placement="bottom">
                  <h4 @click="setDevice(device)">{{device.doorName}}</h4>
                </el-tooltip>
              </p>
              <div class="tags">
                <template v-for="(tag, k) in device.doorStatus">
                  <span class="lx" v-if="tag=='0'" :key="k">离线</span>
                  <!-- <span class="zc" v-else-if="tag=='1'" :key="k">正常</span> -->
              ...
              <div class="info-row">
                ...
                <!-- <span class="version">
                    ...
                    <el-tooltip effect="dark" v-if="device.doorVersion" :content="device.doorVersion" placement="top">
                      <span>{{device.doorVersion}}</span>
                    </el-tooltip>
                    <span v-else>无</span>
                </span>-->
                <!-- </div> -->
            ...
            <div class="device-footer">
              <div @click="handleChange(device)" class="after-line">更换设备</div>
              <div @click="showDelDiglog(device,i)" style="color:#FF534E">删除设备</div>
            </div>
          </div>
        </div>
      </div>
    ...
    <el-dialog title="温馨提醒" width="400px" :visible.sync="showDelDiglogFlag">
      <div style="text-align:center;margin-bottom:40px">
        ...
        <p style="font-size:20px;font-weight:500;color:#343e4c">确认删除此设备吗？</p>
      </div>
...
export default {
  ...
  methods: {
    ...
    handleDel: function (DelData) {
      let self = this;
      let [data, index] = DelData;
      if (data.doorId) {
    ...
    changeMaintainStatus(device) {
      if (this.isOffline(device)) return;
      let self = this;
    ...
    setDevice(data) {
      localStorage.setItem('query',JSON.stringify({id:data.doorId}))
      this.$router.push({
        path: "/equipment/device/setting"
      });
    },
